...
const createuser = async (req, res) => {
  ...
  try {
    ...
    const model = new UserModel({ fullname, address, number, email, role, dob, password: hash });
    await model.save();
  ...
  try {
    ...
    if (enteredEmail.toLowerCase() === "admin@webhub") {
      if (password === "Admin@123") {
        ...
        return res.json({ success: true, message: "Admin login successful", jwtToken: token, fullname: "Admin" });
      } else {
    ...
    if (!existing) return res.status(400).json({ success: false, message: "User does not exist" });
    ...
    res.json({ success: true, message: "Login successful", jwtToken: token, 
